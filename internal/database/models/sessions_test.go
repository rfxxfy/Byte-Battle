...
func testSessionsInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(sessionPrimaryKeyColumns, sessionColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
var (
	sessionDBTypes = map[string]string{`ID`: `integer`, `UserID`: `integer`, `Token`: `character varying`, `ExpiresAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
