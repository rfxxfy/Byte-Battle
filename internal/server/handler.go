...
func toAPIGame(g *models.Game) api.Game {
	result := api.Game{
		...
		CreatedAt: nullTimeOrZero(g.CreatedAt.Time, g.CreatedAt.Valid),
		UpdatedAt: nullTimeOrZero(g.UpdatedAt.Time, g.UpdatedAt.Valid),
	}
	if g.WinnerID.Valid {
		id := g.WinnerID.Int
		result.WinnerId = &id
...
func nullTimeOrZero(t time.Time, valid bool) time.Time {
	if valid {
		return t
	}
	return time.Time{}
