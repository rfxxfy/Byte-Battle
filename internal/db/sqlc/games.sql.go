...
func (q *Queries) CreateGame(ctx context.Context, arg CreateGameParams) (Game, error) {
	row := q.db.QueryRow(ctx, createGame, arg.ProblemID, arg.CreatorID)
	var i Game
	err := row.Scan(
		&i.ID,
		&i.ProblemID,
...
SELECT id, problem_id, creator_id, winner_id, status, started_at, completed_at, created_at, updated_at, current_problem_index FROM games WHERE id = $1 LIMIT 1
`
...
SELECT id, problem_id, creator_id, winner_id, status, started_at, completed_at, created_at, updated_at, current_problem_index FROM games WHERE id = $1 LIMIT 1 FOR UPDATE
`
...
SELECT id, problem_id, creator_id, winner_id, status, started_at, completed_at, created_at, updated_at, current_problem_index FROM games ORDER BY created_at DESC LIMIT $1 OFFSET $2
`
